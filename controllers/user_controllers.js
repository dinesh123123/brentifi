...
const orderDetails=async(req,res)=>{
    ...
    try{
     ...
     const list=await Cart.findOne({"userId":userId,"productId":productId}).populate('productId');
     res.status(200).json({result:"true",message:"required list are",data:list});
}else{
    res.status(400).json({result:"false",message:"required parameters are userId and productId"});
}
}catch(error){
    res.status(400).json({result:"false",message:error.message});
